...
import Header from "../components/Header/page"; // Import the Header component
import "./page.css";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (skippedCases.length > 0) {
          alert(
            `Cases saved successfully! Skipped duplicates: ${skippedCases.join(
              ", "
            )}`
          );
        } else {
      ...
      <table className="table">
        ...
        <tbody>
          {cases.map((c) => (
            <tr key={c.caseId}>
              <td>
                <a href="/case/detail" className="link">
                  {c.caseId}
                </a>
              </td>
              <td>
