...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      const { id, appointmentId, doctor_id, name, date, time, user_id } =
        req.body;
    ...
    if (req.method === "PUT") {
      const { id, name, date, time, status, reason } = req.body;
      if (!id) {
        return res
          .status(400)
          .json({ error: "Appointment ID is required for updating" });
      }
    ...
    if (req.method === "DELETE") {
      ...
      if (!id)
        return res
          .status(400)
          .json({ error: "Appointment ID is required for deletion" });
